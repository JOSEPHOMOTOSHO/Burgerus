...
const findAndDeleteUser = async (email: string): Promise<any> => {
...
const destroyUser = async (payload: string): Promise<any> => {
  const deletedUser = await findAndDeleteUser(payload);
  if (!deletedUser) {
    return generateResponse(false, 'USER_NOT_DELETED', {});
  }
  return generateResponse(true, 'USER_DELETE_SUCCESS', {});
};

export { registerUser, loginUser, verifyToken, updateUser, findUser, destroyUser };
/*To resgister a user
//finduserByEmail
*/
