...
const registerDeleteUserPayloadValidation = (payload: object) => {
  const schema = Joi.object({
    email: Joi.string().email().required(),
  }).required();
...
const validateDeleteUserPayload = (req: Request, res: Response, next: NextFunction) => {
  const validated = registerDeleteUserPayloadValidation(req.body);
  if (validated.error) {
    return handleValidationError(req as any, validated, res);
  }
  return next();
